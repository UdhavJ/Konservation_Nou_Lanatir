...
generateButton.addEventListener('click', () => {
    ...
    generatePuzzle(image, 4); // 4x4 grid
});

function generatePuzzle(imageSrc, gridSize) {
...
function onDrop(event) {
    ...
    const draggedPosition = draggedTile.dataset.position;
    const targetPosition = targetTile.dataset.position;

    // Swap tiles
    draggedTile.dataset.position = targetPosition;
    targetTile.dataset.position = draggedPosition;

    // Re-arrange tiles
    puzzleContainer.insertBefore(draggedTile, targetTile);
    puzzleContainer.insertBefore(targetTile, draggedTile);
